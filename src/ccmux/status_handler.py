"""Status polling and interactive UI detection for active sessions.

Core responsibilities:
  - Background polling loop for status line updates
  - Detect interactive UI transitions (permission prompts)
  - Poll terminal status for all active users
  - Coordinate status updates via message queue

Key functions: start_status_polling(), stop_status_polling(), update_status_message()
"""
...
from .message_queue import enqueue_status_update, get_or_create_queue
...
async def start_status_polling(bot: Bot) -> None:
    """Start the status polling background task."""
    global _status_poll_task
    _status_poll_task = asyncio.create_task(_status_poll_loop(bot))
    logger.info("Status polling task started")


async def stop_status_polling() -> None:
    """Stop the status polling background task."""
    global _status_poll_task
